...
<div class="yith-wcwl-add-button">
    <a href="<?php echo esc_url( add_query_arg( 'add_to_wishlist', $product_id, $base_url ) ) ?>" data-rel="tooltip"
       data-product-id="<?php echo esc_attr( $product_id ) ?>" data-product-type="<?php echo esc_attr($product_type) ?>"
       data-original-product-id="<?php echo esc_attr($parent_product_id) ?>" class="<?php echo esc_attr($link_classes) ?>"
       data-product-title="<?php echo esc_html( $product_title ); ?>" title="<?php echo esc_attr( apply_filters( 'yith_wcwl_add_to_wishlist_title', $label ) ) ?>">
		<?php echo ! empty( $icon ) ? $icon : ''; ?>
        <span><?php echo ! empty( $label ) ? $label : ''; ?></span>
    </a>
</div>