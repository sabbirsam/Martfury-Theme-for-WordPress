...
class Premium_Hscroll extends Widget_Base {
    ...
    // Adding the controls fields for the premium horizontal scroll
    ...
    protected function _register_controls() {
        ...
        $this->add_control('notices', 
            [
                'raw'               => __('<p>Important:</p><ul><li>Please make sure that "Stretch Section" option is disabled for sections below.</li></ul>', 'premium-addons-for-elemeentor'),
        ...
        $temp_repeater->add_control('anchor_id',
		  	[
		     	'label'			=> __( 'Anchor ID', 'premium-addons-for-elementor' ),
...
//        );
        ...
        $this->add_control('nav_arrow_left',
		  	[
		     	'label'         => __( 'Left Arrow Icon', 'premium-addons-for-elementor' ),
        ...
        $this->add_control('nav_arrow_right',
		  	[
		     	'label'         => __( 'Right Arrow Icon', 'premium-addons-for-elementor' ),
		     	'type'          => Controls_Manager::ICONS,
                'default'       => [
                    'library'       => 'fa-solid',
...
//        );
        ...
        $this->add_control('doc_2',
            [
                'type'            => Controls_Manager::RAW_HTML,
                'raw'             => sprintf( __( '%1$s Check the documentation article » %2$s', 'premium-addons-pro' ), '<a href="https://premiumaddons.com/docs/horizontal-scroll-widget-tutorial/" target="_blank" rel="noopener">', '</a>' ),
                'content_classes' => 'editor-pa-doc',
            ]
        );
    ...
    protected function render() {
        ...
        $opacity    = $settings['opacity_transition'];
        ...
        $entrance   = $settings['entrance_animation'];
        
        $keyboard   = $settings['keyboard_scroll'];
        
        $slides     = ! empty ( $settings['slides']['size'] ) ? $settings['slides']['size'] : 1;
        
        $distance   = ! empty ( $settings['distance']['size'] ) ? $settings['distance']['size'] : 0;
        
        $speed      = ! empty ( $settings['scroll_speed'] ) ? $settings['scroll_speed'] : 1;
        
        $hscroll_settings = [
            'id'            => $id,
            'templates'     => $templates,
            'slides'        => $slides,
            'slides_tablet' => empty( $settings['slides_tablet']['size'] ) ? $slides :  $settings['slides_tablet']['size'],
            'slides_mobile' => empty( $settings['slides_mobile']['size'] ) ? $slides :  $settings['slides_mobile']['size'],
            'distance'      => $distance,
            'distance_tablet' => empty( $settings['distance_tablet']['size'] ) ? $slides :  $settings['distance_tablet']['size'],
            'distance_mobile' => empty( $settings['distance_mobile']['size'] ) ? $slides :  $settings['distance_mobile']['size'],
            'snap'          => $snap,
            'disableSnap'   => $disable_snap,
            'speed'         => $speed,
            'speed_tablet' => empty( $settings['scroll_speed_tablet'] ) ? $speed :  $settings['scroll_speed_tablet'],
            'speed_mobile' => empty( $settings['scroll_speed_mobile'] ) ? $speed :  $settings['scroll_speed_mobile'],
            'opacity'       => $opacity,
            'loop'          => $loop,
            'enternace'     => $entrance,
            'keyboard'      => $keyboard,
            'pagination'    => $pagination,
            'arrows'        => $settings['nav_arrows'],
            'dots'          => $settings['nav_dots'],
            'leftArrow'     => $settings['nav_arrow_left']['value'],
            'rightArrow'     => $settings['nav_arrow_right']['value'],
        ];
                        ...
                        ?>
                            <li <?php echo $this->get_render_attribute_string( 'item_' . $index ); ?>>
                                ...
                                <?php if( 'true' === $settings['tooltips'] && ! empty( $tooltips[ $index ] ) ) : ?> 
                                    <span class="premium-hscroll-nav-tooltip"><?php echo $tooltips[ $index ]; ?></span>
                                <?php endif; ?>
                            </li>
                        <?php endforeach; ?>
                    </ul>
